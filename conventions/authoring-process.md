...
| Agent チェック | 判定と反映コミット ID |
| セルフチェック | 秘匿情報・出典・正典・粒度の4点 |
...
- 導入済みの機構: `.github/ISSUE_TEMPLATE/task.yml` + `config.yml`（Issue Form。必須欄が埋まらないと作成できず、blank issue も無効化——md テンプレより強い強制。ただし欄を形だけ埋めた提出までは防げない）、`.github/pull_request_template.md`（記入を想起させる弱い機構。様式は暫定で、運用しながら適宜改訂する）、リポジトリのマージ方式を squash のみに設定
