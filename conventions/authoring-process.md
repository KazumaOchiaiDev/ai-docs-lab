...
| 項目 | 内容 |
|---|---|
| 作成日 | 2026-08-26（v0.2〜v0.4 はオーナーレビューの反映。v0.5: 責務分離により、ブランチ規約を conventions/branching.md へ、Issue / PR の書式を conventions/issue-pr-format.md へ切り出し。v0.6（2026-09-05）: 「実験装置と施策の境界」節を追加） |
...
## 実験装置と施策の境界（何が「施策 + 機構 + 測定」の3点セットを要するか）

このリポジトリで作るものは2層に分かれる。ROADMAP.md の M3 は「施策を1つ導入 → enforcement 機構を実装 → ハーネスで測定 → 実験ログ記録」を1周とするが、この3点セットが要るのは下表の**施策**だけで、**装置**は本規約の一周ループだけで導入できる。区別しないと「規約を作るたびに機構と測定が要るのか」「どこまで作り込んでから測るのか」が曖昧になる。

| 層 | 定義 | 例 | 導入の条件 |
|---|---|---|---|
| **実験装置** | 研究を回すための、リポジトリ自身の規約・テンプレート・ハーネス・機構。測定の前提であり、それ自体は効果測定の対象ではない | 本規約・ブランチ規約・Issue / PR 書式規約（conventions/）、実験ログテンプレート（templates/）、Issue Form・PR テンプレート（.github/）、block-secrets フック（.claude/hooks/）、ゴールデン QA セットと採点手順（harness/、M2 で作成） | 本規約の一周ループで導入する。測定は不要。ただし「規約は機構が守らせる」原則は装置にも適用し、各規約の enforcement 節に導入済み / 未導入を明記する |
| **測定対象への施策** | ハーネスで前後比較する対象、すなわち sample-project/ の資料群と、それを AI が読む形に加える変更 | グロッサリ導入、出典規約の設計書への適用、設計書2層テンプレート、ADR 運用、AI 矛盾検出ワークフロー（ROADMAP.md M3 表の各周） | 3点セット（施策 + enforcement 機構 + ハーネスでの測定）が必須。1周に1施策とし、M2 のベースライン測定より前には導入しない（評価設計 v0.1 §3、research/2026-08-25_評価設計_ドキュメント施策の効果測定.md） |

**境界の判定基準は1つ**: その変更は、M2 でベースラインを測る資料セット（sample-project/ の資料と、ハーネスが AI に渡す資料）に手を入れるか。入れるなら施策、入れないなら装置。研究リポジトリ自身の文書（research/・conventions/・ROADMAP.md・experiments/）はハーネスの入力ではないので、これらへの変更は測定結果に混入せず、装置として扱える。

**装置でもベースラインに影響するもの**: ハーネス自体（QA セット・採点プロンプト・採点モデル）は装置だが、変更すると以前の測定と比較できなくなる。変更した場合は以後の実験を取り直したベースラインと比較する（評価設計 v0.1 §3 の「モデルを更新したらベースラインを取り直す」と同じ扱い）。

**二重の役割を持つ文書**: canon.md（正典の階層）と出典規約（conventions/citation.md）は両層に属する。

- 装置として: 研究リポジトリ自身の文書に**即時適用**する。作成・修正ループ（手順3）の検査基準「正典違反の有無」「技術的主張の出典」はこの適用を指す。適用は人手と手順3の検査で行い、測定は伴わない
- 施策として: sample-project/ の設計書への適用は M3 で機構・測定とセットで行う（出典規約 = 周2、正典の階層と ADR = 周4。ROADMAP.md M3 表）。両文書の enforcement 節が「願望段階」としているのは機構の有無の話であり、装置としての適用はすでに始まっている

装置側の規約（本規約を含む）が研究の速度や品質に効いたかの測定は、この節では扱わない。M3 周6（作成過程の A/B 実験、ROADMAP.md §8）の実験設計時に、リポジトリの Issue 運用データとあわせて検討する。
...
- 未導入（願望段階）: 3 の残りの検査の自動化（lint チェーン = M3 周1〜2）、5 の Agent チェックの機構化（citation-verifier = M3 周2、consistency-checker = M3 周5、CI の claude -p 回帰 = M3 周5）、装置 / 施策の境界の機械検査（sample-project/ を変更する PR が experiments/ の実験ログ更新を伴うことの CI チェック。M2 で対象ディレクトリが揃ってから導入）
